...
A stale pane whose tail shows an interactive permission-confirmation dialog (a compound signature - the confirmation phrase plus numbered options or a "Permission rule ... requires confirmation" line, to avoid false-matching ordinary CLI prompts) is tagged `permission-dialog:` in the wake reason, since it needs a direct keypress response rather than the generic interrupt/relaunch recovery ladder.
...
Only when no matching run exists does it fall back to the pane busy-signature, then whether the pane shows an interactive permission-confirmation dialog (reported as `blocked · pane`), and then the status log; a dead pane without a run reports unknown instead of trusting a stale log.
