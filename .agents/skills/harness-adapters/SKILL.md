...
**Permission-confirmation dialogs cannot be silently pre-approved; nothing overrides a matching global `ask` rule (task crew-rmrf-fix-q3; verified 2026-07-09/10, Claude Code 2.1.205).**
A crewmate is launched with `--dangerously-skip-permissions`, but the global `~/.claude/settings.json` deliberately keeps some patterns (e.g. `Bash(rm -rf *)`) in `ask` as a standing safety net, and a command matching one of those patterns still shows an interactive "Do you want to proceed?" dialog even under the skip-permissions flag.
Two independent mechanisms were tried and empirically ruled out as ways to suppress that dialog for a legitimate, narrow case, both tested with real interactive sessions (not `claude -p`, which behaves more conservatively and is not a valid stand-in - see below):
1. A worktree-scoped `permissions.allow` entry in `.claude/settings.local.json` (e.g. `Bash(rm -rf /abs/worktree/path/*)`) does NOT suppress the prompt for a single-line command that literally starts with `rm -rf `, no matter how specific the pattern is or which settings tier it comes from - confirmed on multiple clean single-line tests, including through a real `bin/fm-spawn.sh`-launched crewmate.
2. A `PreToolUse` hook returning `{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"allow"}}` (the documented force-approve schema) also does NOT suppress the prompt for a matching `ask` rule, confirmed the same way - contrary to third-party documentation summaries claiming hooks run before rule evaluation. A hook CAN move a decision the other way - force `ask`/`deny` even when a setting would otherwise silently allow - so hooks are a valid tool for narrowing an existing allow, never for widening past an explicit `ask` match.
Separately, and NOT a supported escape hatch: a *multi-line/compound* command whose full literal text does not start with `rm -rf ` (e.g. `WT="$(pwd -P)"` on one line, `rm -rf "$WT/..."` on the next) does not match the `ask` pattern's literal-prefix check at all, regardless of any allow rule, and runs unprompted under bypass-permissions' default-allow for anything not explicitly `ask`-matched. This is what produced an initial false-positive "the allow rule worked" observation before more careful re-testing traced the real cause to this prefix mismatch, unrelated to any allow rule. Relying on this shape to dodge the prompt is exploiting a matcher gap, not a real technical control, and is not something firstmate does deliberately.
The only mechanism that has been found to actually suppress this dialog is narrowing or removing the matching pattern in the global `~/.claude/settings.json` itself - outside any repo, a captain decision, not a per-task file firstmate or a crewmate can generate. Given that, firstmate's approach is not to suppress this dialog: `bin/fm-crew-state.sh` and `bin/fm-watch.sh` detect a pane parked at one (`FM_TMUX_PERMISSION_DIALOG_REGEX_DEFAULT` in `bin/fm-tmux-lib.sh`, matching `Do you want to proceed?`) and report it distinctly (`state: blocked · source: pane`; a stale wake reason carries a `permission-dialog:` tag) so it is never mistaken for routine idleness, and the `stuck-crewmate-recovery` skill answers it directly rather than interrupting or relaunching.
When verifying Claude Code permission-rule behavior of any kind, use a real interactive session (a live tmux pane, ideally via a real `bin/fm-spawn.sh` launch), never `claude -p` — it was directly responsible for an earlier, wrong conclusion in this same investigation.
