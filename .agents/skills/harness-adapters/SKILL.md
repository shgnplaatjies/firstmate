...
**Permission precedence: a worktree-scoped `allow` DOES suppress a matching global `ask`, but only in a real interactive session - `claude -p` (print/headless mode) is not a faithful test of this (verified 2026-07-09, Claude Code 2.1.205).**
A crewmate is launched with `--dangerously-skip-permissions`, but that flag alone does not suppress an explicit `ask` rule match - the global `~/.claude/settings.json` deliberately keeps some patterns (e.g. `Bash(rm -rf *)`) in `ask` as a standing safety net, and any such match still prompts even under the skip-permissions flag when nothing else resolves it.
`bin/fm-spawn.sh` writes a crewmate-worktree-scoped `allow` entry (`Bash(rm -rf <worktree's own absolute path>/*)`) into that task's `.claude/settings.local.json` alongside the existing turn-end `Stop` hook, specifically so routine worktree-scoped cleanup (deleting `node_modules`, `.terraform`, a scratch trace dir) does not park an unattended crewmate at a confirmation dialog it cannot answer itself.
Verified end to end with a real `bin/fm-spawn.sh` launch (tmux backend, real treehouse worktree, real generated `settings.local.json`): inside the live interactive session, `rm -rf "$(pwd -P)/scratch-test/..."` (the worktree's own absolute path, matching the generated `allow` entry) ran immediately with no prompt, while `rm -rf` on a path outside the worktree (not matching any `allow` entry) still correctly triggered the interactive confirmation - so the safety net for anything outside a crewmate's own worktree, and the hard denies on `rm -rf /`, `~`, `$HOME`, are untouched.
This means an `allow` rule CAN win against a matching global `ask` rule for the same command, contrary to what an earlier round of testing this same day concluded.
That earlier, incorrect conclusion came from testing exclusively via `claude -p --dangerously-skip-permissions "<prompt>"` (headless print mode, no PTY): in that mode a matching `allow` rule never won against a matching `ask` rule no matter its source or specificity (tried as a worktree-local `settings.local.json` entry, a `--settings`-loaded file combining both rules, the CLI `--allowedTools` flag, and an exact literal `allow` pattern) - print mode appears to resolve an ask/allow conflict more conservatively (favoring a block) than a genuine interactive session does, so it is not a valid stand-in for how a real crewmate's interactive pane will behave.
**When verifying Claude Code permission-rule behavior, use a real interactive session (a live tmux pane via `bin/fm-spawn.sh`, or `claude` with a real PTY), not `claude -p`.**
